...
sample_export = function(sormas_db, fromDate, toDate){
  # leading sample table linked to cases only because a sample can be referenced by contacts, cases, event participants
  # the goal is to export sample for each entity type and stack the tables 
  # this implementation is for samples lined to cases only, others can be added later 
  ...
  pathogentestresult, associatedcase_id, samplingreason, samplepurpose, shipped, received, referredto_id,specimencondition, samplesource, shipmentdate, 
  receiveddate, samplematerial
                        FROM public.samples
                        WHERE deleted = FALSE and associatedcase_id IS NOT NULL and reportdatetime between '", fromDate, "' and '", toDate, "' ")
  sample_forcases = dbGetQuery(sormas_db,querySample)
...
if(!dataframe_is_empty(sample_forcases)){
  #loading cases
  queryCase <- base::sprintf("SELECT uuid AS case_uuid, id AS case_id, disease AS disease_case, responsibleregion_id AS region_id_case, responsibledistrict_id AS district_id_case,
  caseclassification AS case_classification
                        FROM public.cases
                        WHERE deleted = FALSE and id in (%s)", paste("'", unique(sample_forcases$associatedcase_id), "'",collapse=",") )
  ...
  ret = sample_forcases %>% 
    ...
    dplyr::mutate(disease_sample = disease_case) %>% # signing disease of samples as that of case. include disease of other entities (contacts, evetn part) when adding them to the sample export
